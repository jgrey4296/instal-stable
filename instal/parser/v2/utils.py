...
event_kws      = pp.MatchFirst(kw(x.replace('_', ' ')) for x in list(ASTs.EventEnum.__members__.keys()) + ["exo", "inst", "viol", "external"])
fluent_kws     = pp.MatchFirst(kw(x.replace('_', ' ')) for x in list(ASTs.FluentEnum.__members__.keys()) + ["x", "obl"])
...
# TODO maybe compare to consts
COMPARISON  = TERM("head") + op_lits("op") + TERM("rhs")
COMPARISON.set_parse_action(lambda s, l, t: ASTs.ConditionAST(t['head'], False, operator=t['op'], rhs=t['rhs'], parse_loc=(pp.lineno(l, s), pp.col(l, s))))
