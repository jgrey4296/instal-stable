...
from instal.instalexceptions import (InstalParserError,
                                     InstalParserSyntaxError,
                                     InstalParserTypeError)
...
class InstitutionParser(InstalParser):
    """
        InstalParser
        Converts .ial file to InstAL IR. Use parser_institution(self, data: str) where data is the IAL text.
    ...
    def p_cross_fluent_declaration_gpow(self, p):
        ...
        """
        self.cross_generation_fluents = ([[p[5], p[7], p[9]]]
                                         + self.cross_generation_fluents)
    ...
    def p_cross_fluent_declaration_tpow(self, p):
        ...
        self.cross_termination_fluents = ([[p[5], p[7], p[9]]]
                                          + self.cross_termination_fluents)
        p[0] = [p[1]]
    ...
    def p_cross_fluent_declaration_ipow(self, p):
        ...
        self.cross_initiation_fluents = ([[p[5], p[7], p[9]]]
                                         + self.cross_initiation_fluents)
        p[0] = [p[1]]
