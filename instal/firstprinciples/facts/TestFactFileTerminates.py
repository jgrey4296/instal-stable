...
class FactFileTerminates(InstalTestCase):
    ...
    def test_iaf_kill_pow_institutional(self):
        ...
        in_fact_holdsat_condition = [{"holdsat": ["holdsat(pow(ex_a(foo)),fact)"]}] + [{"notholdsat": [
            "holdsat(pow(ex_a(foo)),fact)"]}] * 2
        self.assertEqual(runner.run_test(query_file="facts/kill_event.iaq", fact_files=["facts/pow_ex_true.iaf"],
                                         verbose=self.verbose,
    ...
    def test_iaf_kill_obligation(self):
        ...
        in_fact_holdsat_condition = [{"holdsat": ["holdsat(obl(in_a(foo), in_b(foo), vi_c(foo)),fact)"]}] + [{
            "notholdsat": [
                "holdsat(obl(in_a(foo), in_b(foo), vi_c(foo)),fact)"]}] * 2
        self.assertEqual(
