...
class InstalState(object):
    def __init__(self):
        self.metadata = {} # type: Dict
        self.holdsat = [] # type: List[Symbol]
        self.occurred = [] # type: List[Symbol]
        self.observed = [] # type: List[Symbol]

    @classmethod
    def state_from_json(cls, json : dict) -> "InstalState":
        state = InstalState()
    ...
    @classmethod
    def state_from_list(cls, lst : List[Symbol], metadata : dict = None) -> "InstalState":
        state = InstalState()
    ...
    def check_trace_for(self, conditions : dict, verbose=2):
