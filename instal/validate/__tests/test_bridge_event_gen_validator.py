...
class TestBridgeEventGenValidator(unittest.TestCase):
    ...
    def test_basic_pass(self):
        ...
        data = parser.parse_bridge(text, parse_source=file_name)
    ...
    def test_basic_with_rules_fail(self):
        ...
        Validator no reports are generated by default
        """
        file_name_bridge = "bridge_event_gen_rules.iab"
        file_name_insts  = "bridge_basic_sourceSinks.ial"
        runner           = validate.InstalValidatorRunner([ BridgeEventGenValidator() ])

        bridge_text      = data_path.joinpath(file_name_bridge).read_text()
        inst_text        = data_path.joinpath(file_name_insts).read_text()

        bridge_data      = parser.parse_bridge(bridge_text, parse_source=file_name_bridge)
        inst_data        = parser.parse_institution(inst_text, parse_source=file_name_insts)
