...
class InstalPDFReporter(Reporter):
    """
        InstalPDFTracer
    ...
    def trace_to_file(self, remove_permpows=True):
        ...
        # TODO use __data.pdf.pdf_header.latex
        latex_trace_header = None

        observed = {t - 1: self.trace.trace[t].observed
        ...
        for t in range(0, len(self.trace.trace)):
            ...
            if remove_permpows:
                holdsat[t] = sorted((f for f in self.trace.trace[t].holdsat
                                     if not (f.arguments[0].name in ["perm", "pow", "ipow", "gpow", "tpow"])),
                                    key=lambda x: x.arguments[0].name)
            else:
