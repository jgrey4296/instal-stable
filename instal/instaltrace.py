...
def instal_trace_preprocess_with_args(args, unk) -> None:
    ...
    if os.path.isdir(args.json_file):
        ...
        for t in os.listdir(args.json_file):
            traces.append(InstalStateTrace.state_trace_from_json_file(os.path.join(args.json_file,t)))
    else:
        is_dir = False
        traces.append(InstalStateTrace.state_trace_from_json_file(args.json_file))
    ...
    for trace in traces:
        if args.trace_file:
            if is_dir:
                ...
                filename = args.trace_file
            instal_pdf_trace(trace, filename)
